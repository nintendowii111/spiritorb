...
(function (_) {
  ...
  _.blendNames = ["Normal", "Add", "Multiply", "Screen", "Overlay", "Darken", "Lighten", "Color Dodge", "Color Burn", "Hard Light", "Soft Light", "Difference", "Exclusion", "Hue", "Saturation", "Color", "Luminosity"];
  ...
  HUDManager.createFilelist = function (id, folder, value, includeNone) {
    ...
    if (includeNone) {
      if (value === "N\n\nONE") select = 'selected';else select = '';
      result += '<option value="N\n\nONE" ' + select + '>[NONE]</option>';
    }

    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      if (file === value) select = 'selected';else select = '';
  ...
  Sprite_HUDObject.prototype.setupAnimationInfo = function (aniInfo) {
    //Dynamic Visibility
    this._condition = '';
    this._isVisible = true; //X Animation
    ...
    this._yDir = 1;
    this._yOffset = 0; //Scale Animation
    ...
    this._sCheck = 1;
    this._baseXScale = 1;
    this._baseYScale = 1; //Rotation Animation
  ...
  Sprite_HUDText.getHtml = function (data) {
    ...
    const sele = ['', '', ''];
    if (align === 'left') sele[0] = 'selected';else if (align === 'center') sele[1] = 'selected';else if (align === 'right') sele[2] = 'selected';
  ...
  Sprite_HUDText.prototype.initialize = function (info) {
    ...
    this.properties = ["Layer", "Condition", "Max Width", "Font", "Align", "Font Size", "Text Color", "Text Alpha", "Outline Color", "Outline Alpha", "Value"];
  ...
  Sprite_HUDShape.getHtml = function (data) {
    ...
    const sele = ['', ''];
    if (shape === 'Rectangle') sele[0] = 'selected';else if (shape === 'Circle') sele[1] = 'selected';
    const sele2 = ['', '', '', '', ''];
    if (fill === 'solid') sele2[0] = 'selected';else if (fill === 'horizontal') sele2[1] = 'selected';else if (fill === 'vertical') sele2[2] = 'selected';else if (fill === 'radical') sele2[3] = 'selected';else if (fill === 'max radical') sele2[4] = 'selected';
    const blendArray = [];
    ...
    return `${HUDManager.createTitle(data.id, Sprite_HUDShape._label)}
			...
			<table>
				...
				${HUDManager.createInput("Width", width)}
				${HUDManager.createInput("Height", height)}
				${HUDManager.createSelect("Fill Style", ["solid", sele2[0], "Solid"], ["horizontal", sele2[1], "Horizontal Gradient"], ["vertical", sele2[2], "Vertical Gradient"], ["radical", sele2[3], "Radial Gradient"], ["max radical", sele2[4], "Max Radial Gradient"])}
  ...
  Sprite_HUDShape.prototype.initialize = function (info) {
    Sprite_HUDObject.prototype.initialize.call(this, new Bitmap(1, 1), info);
    this.properties = ["Layer", "Condition", "Shape", "Width", "Height", "Fill Style", "Blend", "Color 1", "Color 1 Alpha", "Color 2", "Color 2 Alpha", "Outline Size", "Outline Color", "Color 3 Alpha"];
  ...
  Sprite_HUDGauge.getHtml = function (data) {
    ...
    const sele2 = ['', '', '', ''];
    if (fill === 'left') sele2[0] = 'selected';else if (fill === 'right') sele2[1] = 'selected';else if (fill === 'up') sele2[2] = 'selected';else if (fill === 'down') sele2[3] = 'selected';
    ...
    return `${HUDManager.createTitle(data.id, Sprite_HUDGauge._label)}
			...
			<table>
				...
				${HUDManager.createInput("Width", width)}
				${HUDManager.createInput("Height", height)}
				${HUDManager.createSelect("Style", ["left", sele2[0], "Left"], ["right", sele2[1], "Right"], ["up", sele2[2], "Up"], ["down", sele2[3], "Down"])}
				${HUDManager.createColor("Color 1", color1, "Color 1 Alpha", color1A)}
				${HUDManager.createColor("Color 2", color2, "Color 2 Alpha", color2A)}
  ...
  Sprite_HUDGauge.prototype.initialize = function (info) {
    Sprite_HUDObject.prototype.initialize.call(this, new Bitmap(1, 1), info);
    this.properties = ["Cur. Value", "Max Value", "Condition", "Layer", "Width", "Height", "Style", "Color 1", "Color 1 Alpha", "Color 2", "Color 2 Alpha", "Back Color", "Color 4 Alpha", "Outline Size", "Outline Color", "Color 3 Alpha"];
  ...
  Sprite_HUDImageGauge.prototype.initialize = function (info) {
    Sprite_HUDObject.prototype.initialize.call(this, new Bitmap(1, 1), info);
    this.properties = ["Cur. Value", "Max Value", "Condition", "Layer", "Scale X", "Scale Y", "Style", "Main Image", "Back Image"];
  ...
  Sprite_HUDFace.getHtml = function (data) {
    ...
    const sele = ['', ''];
    if (mask === '0') sele[0] = 'selected';else if (mask === '1') sele[1] = 'selected';
    ...
    return `${HUDManager.createTitle(data.id, Sprite_HUDImageText._label)}
			<table>
				...
				${HUDManager.createSelect("Mask", ["0", sele[0], "None"], ["1", sele[1], "Circle"])}
				${HUDManager.createRefresh()}
			</table>`;
  };
  /*
