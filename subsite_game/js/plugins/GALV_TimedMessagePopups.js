...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.1.9) Display popup message boxes on screen that do not stop gameplay and auto close after a set time.
 * 
...
Game_Interpreter.prototype.createCaption = function(cap,pos,data) {
	...
	SceneManager._scene.createCaptionWindow(target,time,txtArray,data,delay,windowskin,yoffset);
};
...
Scene_Map.prototype.createCaptionWindow = function(target,time,textArray,data,delay,windowskin,yoffset) {
	var p = new Window_GalvCaption(target,time,textArray,data,delay,windowskin,yoffset);
...
Scene_Battle.prototype.createCaptionWindow = function(target,time,textArray,data,delay,windowskin,yoffset) {
	var p = new Window_GalvCaption(target,time,textArray,data,delay,windowskin,yoffset);
	this._captionWindows.push(p);
	this.addWindow(p);
};
...
Window_GalvCaption.prototype.initialize = function(target,time,textArray,data,delay,windowskin,yoffset) {
	...
	if (SceneManager._scene.constructor.name == 'Scene_Map') {
		this.setMapTarget(target,yoffset);
	} else {
		this.setBattleTarget(target,yoffset);
	};
...
Window_GalvCaption.prototype.setMapTarget = function(target,yoffset) {
...
Window_GalvCaption.prototype.setBattleTarget = function(target,yoffset) {
	Galv.Mpup.thisEvent = null;
